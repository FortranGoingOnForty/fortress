...
contains
    ...
    function list_directory(path) result(entries)
        ...
        do
            ...
            ! Check if it's executable (but not directories)
            if (.not. entries(count)%is_dir) then
                entries(count)%is_executable = is_executable(full_path)
            else
                entries(count)%is_executable = .false.
            end if
    ...
    function is_executable(path) result(is_exec)
        character(len=*), intent(in) :: path
        logical :: is_exec
        integer :: stat
        ...
        ! Use test command to check if it's executable
        call execute_command_line("sh -c 'test -x """ // trim(path) // """'", &
                                  exitstat=stat, wait=.true.)
        is_exec = (stat == 0)
    end function is_executable
